...
function ImagesForm() {
  ...
  // const loadImageData = () => {
  //   if (localStorage.getItem("PageName") == "Edit Image") {
  //     let subCatData = JSON.parse(localStorage.getItem("ImageDataForEdit"));
  //   }
  // };

  const onSubmit = (data) => {
    if (localStorage.getItem("PageName") == "Add New Image") {
      addNewRecord(data);
    } else if (localStorage.getItem("PageName") == "Edit Image") {
      updateRecord(data);
    }
  };
  ...
  const addNewRecord = (data) => {
    ...
    fd.append("ProdImage", data.ProdImage[0]);
    Service.addNewImage(fd).then((res) => {
      if (res.data.success) {
        setShowMsg(true);
  ...
  const updateRecord = (data) => {
    setIsLoading(true);
    var imageData = JSON.parse(localStorage.getItem("ImageDataForEdit"));
    let fd = new FormData();
    fd.append("ImgId", imageData.ImgId);
    fd.append("ProdId", data.ProdId);
    fd.append("ProdImage", data.ProdImage[0]);
    Service.updateImage(fd).then((res) => {
      if (res.data.success) {
        setShowMsg(true);
        setMsgClass("alert alert-success alert-dismissible");
        setMsgText(res.data.success);
        ...
        <div className="row">
          ...
          <div className="col-sm-6">
            <div className="form-group">
              ...
              />
              {errors.ProdImage && (
                <span className="text-danger">{errors.ProdImage.message}</span>
              )}
            </div>
            {/* End .form-group */}
          </div>
